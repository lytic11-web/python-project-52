...
class TaskFilter(django_filters.FilterSet):
    ...
    status = django_filters.ModelChoiceFilter(
        queryset=None, label="Статус", empty_label="---------"
    )
    executor = django_filters.ModelChoiceFilter(
        queryset=None, label="Исполнитель", empty_label="---------"
    )
    labels = django_filters.ModelChoiceFilter(
        queryset=None, label="Метка", empty_label="---------"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.filters["status"].queryset = Task._meta.get_field(
            "status"
        ).related_model.objects.all()
        self.filters["executor"].queryset = User.objects.all()
        self.filters["labels"].queryset = Task._meta.get_field(
            "labels"
        ).related_model.objects.all()
